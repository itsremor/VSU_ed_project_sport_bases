$(function () {
    ...
    $('#slider-range').append($('.ui-slider-range'))
    
    // Apply initial values to the range container
    $('.range1').html('<span class="range-value">' + $('#slider-range').slider("values", 0).toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,") + '</span>');
    
    $('.range2').html('<span class="range-value">' + $("#slider-range").slider("values", 1).toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,") + '</span>');
    ...
    $('#slider-range').slider({
        slide: function (event, ui) {
            ...
            $('.range1').html('<span class="range-value">' + ui.values[0].toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,"));
    
            $('.range2').html('<span class="range-value">' + ui.values[1].toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,") + '</span>');
